...
AVERAGEIFS = Function(statistical.average_ifs, shape=SCALAR_SHAPE)
...
COMBINA = numba_disabled(Function(scipy.special.comb).append([False, True]))
...
#           Matrix Functions
######################################
MMULT = Function(np.dot).cast_inputs(np.float_)  # numba matrix mult only works on floats.
...
ATAN2 = Function(np.arctan2).cast_inputs(np.float_).reorder_inputs([1, 0])
